...
> Grizzly has been superseded by the [gcx CLI](https://github.com/grafana/gcx) tool.
> The `grizzly.grafana.com/v1alpha1` API group used by this library was Grizzly-specific and is not the native Grafana API.
> All users are encouraged to migrate to the native Grafana Kubernetes API (e.g. `dashboard.grafana.app/v1`, `folder.grafana.app/v1`) using [gcx](https://github.com/grafana/gcx) or other tooling such as Terraform, grafana-operator, Crossplane, or git-sync.
> Note: the native Grafana Kubernetes API requires Grafana 12 or later.
